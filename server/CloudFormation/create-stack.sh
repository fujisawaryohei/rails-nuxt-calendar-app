...
aws cloudformation create-stack --stack-name TaskDefinitionStack --template-body file://"$(pwd)"/Stack/ElasticContainerService/TaskDefinition.yml --capabilities CAPABILITY_NAMED_IAM
aws cloudformation wait stack-create-complete --stack-name TaskDefinitionStack

# Clusterスタックの作成
aws cloudformation create-stack --stack-name ClusterStack --template-body file://"$(pwd)"/Stack/ElasticContainerService/Cluster.yml
aws cloudformation wait stack-create-complete --stack-name ClusterStack

# Serviceスタックの作成
aws cloudformation create-stack --stack-name ServiceStack --template-body file://"$(pwd)"/Stack/ElasticContainerService/Service.yml
aws cloudformation wait stack-create-complete --stack-name ServiceStack

# Route53スタックの作成 TODO: Fargate IPの固定化
# aws cloudformation create-stack --stack-name Route53Stack --template-body file://"$(pwd)"/Stack/Route53.yml